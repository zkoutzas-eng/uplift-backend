...
app.post('/api/employees', async (req, res) => {
  ...
  const { count } = await supabase.from('employees').select('*', { count: 'exact', head: true }).eq('company_id', company_id);
...
// into THEIR timezone, then compare against their chosen send_time.
// This means an Australian employee who picked 9:00 AM gets their email
// at 9:00 AM Sydney time, not 9:00 AM UTC (which would be 7:00 PM AEST).
cron.schedule('* * * * *', async () => {
  const now = new Date();
  ...
  for (const employee of employees) {
    ...
    // Convert current UTC time into the employee's local timezone
    ...
    // Only send if this matches their chosen send time
